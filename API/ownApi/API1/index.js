...
app.get("/random", async (req, res) => {
  try {
    res.status(200).json(jokes[Math.floor(Math.random() * jokes.length)]);
  } catch (error) {
...
app.get("/jokes/:id", async (req, res) => {
  try {
    const apiUrl = req.params.id;
    const joke = jokes[apiUrl - 1];
    const response = res.json(joke);
  } catch (error) {
    res.status(500).json(error.message);
  }
});
...
app.get("/filter?:id", async (req, res) => {
  try {
    ...
    console.log(Url);
    res.json(jokes.filter((joke) => joke.jokeType === Url));
  } catch (error) {
    res.status(500).json(error.message);
  }
});
...
app.post("/jokes", async (req, res) => {
  try {
    ...
    jokes.push({ jokeText: text1, jokeType: type1 });
    res.json(jokes);
  } catch (error) {
    res.status(500).json(error.message);
  }
...
app.patch("/jokes/:id", async (req, res) => {
  try {
    ...
    const joke = jokes.find((event) => event.id == Id);
    console.log(joke);
